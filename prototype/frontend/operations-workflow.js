...
export function collectionStage(order, batch, report, issues = []) {
  if (!batch && order?.status==='已通过') return {step:2,label:'材料审核通过，等待后续处理'};
...
export function createCollectionWorkflow(ctx) {
  ...
  function render(order, batch) {
    ...
    let html = `<section class="panel oc-panel oc-journey"><div class="oc-section-title"><div><h2>${order?'报送办理进度':'数据办理进度'}</h2><p>当前：<strong>${esc(stage.label)}</strong></p></div>${tag(batch?.status==='已入库'?'已入库':order?.status||batch?.status||'待报送')}</div><ol class="oc-steps">${(order?['材料报送','登记审核','质检与整改','确认入库']:['材料登记','批次准备','质检与整改','确认入库']).map((name,i)=>`<li class="${i<stage.step?'done':i===stage.step?'current':''}" ${i===stage.step?'aria-current="step"':''}><span>${i<stage.step?'✓':i+1}</span>${name}</li>`).join('')}</ol>`;
    ...
    if(stage.step===3) {
      ...
      if(targets.length)html+=wrapForm('oc-flow-import','确认目标与字段对应关系',field('datasetId','目标空表',chosen?.id||'','select',[['','请选择目标空表'],...targets.map(x=>[x.id,x.name])])+`<div id="oc-flow-mapping" class="ct-full">${mapping(batch,chosen,previous?.mapping)}</div><p class="ct-full muted">同名字段已预填，请核对。执行前会再次检查最新数据、标准、规则及整改状态。</p>`,'校验并入库');
      else html+='<p class="oc-blocked">质量检查已通过。请先在下方创建目标空表，再确认字段映射。</p>';
