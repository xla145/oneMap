...
export const operationMenu = operationGroups.map(([key,label,pages])=>['op-'+key,label,'layers',pages.map(([path])=>'operations/'+path)]);
export const operationLabels = Object.fromEntries(operationGroups.flatMap(g=>g[2].map(([path,label])=>['operations/'+path,label])));
const descriptors = {
...
export function createOperationsCenter(ctx) {
  ...
  const state=()=>{const [hash,search='']=location.hash.split('?');const parts=hash.replace(/^#\//,'').split('/').slice(2);let path=parts.slice(0,2).join('/');if(['overview','todos'].includes(parts[0]))path=parts[0];const n=path.includes('/')?2:1;return {path:path||'overview',id:decodeURIComponent(parts[n]||''),edit:parts[n+1]==='edit',params:new URLSearchParams(search)};};
  const base=()=>'/'+ctx.state().mode+'/operations/';
  const href=(path,id='',edit=false)=>'#'+base()+path+(id?'/'+encodeURIComponent(id):'')+(edit?'/edit':'')+(state().params.toString()?'?'+state().params:'');
  ...
  const pages=operationGroups.flatMap(g=>g[2]);
  ...
  function associations(r){
    ...
    let html=Object.entries(paths).filter(([key])=>r[key]).map(([key,path])=>key==='ingestionId'?`<span class="tag">批次：${esc(refName(key,r[key]))}</span>`:link(path,labels[key]+' · '+refName(key,r[key]),r[key],'small')).join('');
  ...
  function filterRows(rows){const p=state().params,q=p.get('q')||'',status=p.get('status')||'';return rows.filter(r=>(!q||JSON.stringify(r).toLowerCase().includes(q.toLowerCase()))&&(!status||(status==='未办结'?r.status!=='已办结':status==='未关闭'?r.status!=='已关闭':r.status===status)));}
  function search(rows){const p=state().params;return `<form id="oc-search" class="ct-search"><input name="q" aria-label="搜索运营记录" placeholder="搜索名称、编号、单位" value="${esc(p.get('q')||'')}"><select name="status" aria-label="业务状态"><option value="">全部状态</option>${[...new Set([...(rows.some(r=>r.status==='已办结')?['未办结']:[]),...(rows.some(r=>r.status==='已关闭')?['未关闭']:[]),p.get('status'),...rows.map(r=>r.status)].filter(Boolean))].map(s=>`<option ${p.get('status')===s?'selected':''}>${esc(s)}</option>`).join('')}</select><button class="btn primary">查询</button>${button('重置','reset')}</form>`;}
  function list(path){
  ...
  function overview(path){
    ...
    let title=pages.find(p=>p[0]===path)?.[1]||'运营总览';
    let html=head(title,'本地业务台账实时汇总 · 当前全部范围 · 更新于 '+(D().serverTime||'') ,link('collection/tasks','新建归集任务','new','primary')+button('执行业务巡检','inspect'));
    html+=cards([
  ...
  function detail(path,r){
    ...
    if(path==='collection/tasks')html+=`<section class="panel oc-panel"><h2>单位报送工单</h2>${table(['工单','报送人','状态','批次'],O().orders.filter(x=>x.taskId===r.id).map(x=>tr(link('collection/orders',x.name,x.id,'text'),esc(x.assigneeName),tag(x.status),esc(x.ingestionId||'尚未审核通过'))))}</section>`;
    ...
    if(path==='monitoring/alerts'){const batch=C().ingestions.find(x=>x.id===r.targetId);html+=`<section class="panel oc-panel"><h2>异常来源</h2>${batch?.qualityRunId?link('quality/results','查看对象最近质检报告',batch.qualityRunId):r.kind==='服务依赖异常'?link('publishing/apis','查看异常服务',r.targetId):'<a class="btn" href="#/admin/center-resources">查看交付台账</a>'}<a class="btn" href="#/admin/portal-audit">统一日志审计</a></section>`;}
  ...
  function assetDetail(r,html){
    ...
    const sections=['基本信息','技术元数据','标准与质量','来源与关系','发布与共享','AI 标注','版本与记录'];const selected=state().params.get('view')||sections[0];
    ...
    if(selected==='技术元数据')html+=table(['字段','中文名','类型','说明'],(r.fields||[]).map(f=>tr(esc(f.name),esc(f.label),esc(f.type),esc(f.description))))+`<p>坐标系：${esc(r.crs||'未填写')}</p>${button('预览本地数据','preview',r.id)}`;
    if(selected==='标准与质量')html+=datasets.map(d=>associations(d)).join('')+table(['报告','采用标准','质量结论'],q.map(x=>tr(link('quality/results',x.name,x.id,'text'),esc(x.standardSnapshot?.name||'—')+' v'+(x.standardSnapshot?.version||'—'),tag(x.status))));
    if(selected==='来源与关系')html+=datasets.map(d=>link('warehouse/tables',d.name,d.id)).join('')+services.map(x=>link(x.protocol==='本地数据接口'?'publishing/apis':'publishing/layers',x.name,x.id)).join('')+`<p>资源关系：${(r.relations||[]).map(id=>esc(refName('resourceId',id))).join('、')||'暂无'}</p><a class="btn" href="#/admin/intelligence-links?asset=${encodeURIComponent('resources:'+r.id)}&view=draft">跨中心关联与影响</a>`;
    if(selected==='发布与共享')html+=`<p>技术编目、服务发布、共享策略与个人授权分别管理。当前共享策略：${esc(r.sharingPolicy||'申请使用')}。</p>${services.map(x=>link(x.protocol==='本地数据接口'?'publishing/apis':'publishing/layers',x.name,x.id)).join('')}${btn('维护共享目录','ct-sharing',r.id)}<a class="btn" href="#/admin/grants">查看授权台账</a>`;
    if(selected==='AI 标注')html+=`<p>语义别名：${esc(r.aliases||'尚未标注')}</p><p>${esc(r.description||'尚无语义说明')}</p><a class="btn primary" href="#/admin/metadata-annotation?resourceId=${encodeURIComponent(r.id)}">前往智能中心标注同一资产</a>`;
    if(selected==='版本与记录')html+=`<p>编辑修订 ${r.rev} · 已发布版本 ${r.published?.version||'无'}</p><pre class="ct-json">${esc(JSON.stringify(r.versions||[],null,2))}</pre>`;
  ...
  function schema(path,r){
    ...
    if(path==='collection/imports')html+=f('ingestionId','来源批次 *','select',options(C().ingestions.filter(x=>x.status!=='已入库')))+f('datasetId','目标本地空表 *','select',options(C().datasets.filter(x=>x.rowCount===0)))+f('mapping','字段映射 *（JSON：目标字段 → 来源字段）','json',[],{id:'id',name:'name',region:'region',area:'area'})+'<p class="ct-full notice">首批只导入空表；不执行全量覆盖。Excel、GDB、MDB 解析和外部数据库写入尚未接入。</p>';
  ...
  function render(){
    ...
    if(!O().canManage&&!userAllowed.includes(path)){html=head('暂无管理权限','当前身份仅可办理本人的报送和整改工单。',link('todos','我的待办'));}
      ...
      const record=id==='new'?{}:rowsFor(path).find(x=>x.id===id);
    ...
    $('#main').innerHTML=`<div class="ct-workspace oc-workspace">${html}</div>`;bind();return true;
  }
  ...
  function bind(){
    ...
    preview();$('#oc-editor')?.addEventListener('change',preview);
