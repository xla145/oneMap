...
export function createPublicPortal(ctx){
  ...
  function home(){
    ...
    const headline=String(config.title||'自然资源一张图').split(/[，,]/);
    const resourceCard=(r,i)=>`<article class="home-resource"><div class="home-resource-top"><span class="tile">${icon(r.type==='图层服务'?'layers':'database')}</span><span>${esc(r.type)}</span><small>${String(i+1).padStart(2,'0')}</small></div><h3>${link('data',esc(r.name),r.id,'')}</h3><p>${esc(r.description||r.summary||'查看资源信息与使用说明')}</p><dl><div><dt>覆盖范围</dt><dd>${esc(r.region||'全区')}</dd></div><div><dt>提供单位</dt><dd>${esc(r.source||'未提供')}</dd></div><div><dt>更新日期</dt><dd>${esc(r.updated?.slice(0,10)||'未提供')}</dd></div></dl><div class="home-resource-bottom">${r.access?tag(r.access):'<span>资源信息</span>'}${link('data','查看详情 '+icon('arrow'),r.id,'pub-more')}</div></article>`;
    return `<div class="home-editorial">
      <section class="pub-hero"><div class="portal-container pub-hero-inner">
        <div class="home-hero-copy"><span class="pub-eyebrow"><i></i>内蒙古自治区 · 自然资源公共服务</span><h1>${esc(headline[0])}${headline.length>1?`<span>${esc(headline.slice(1).join('，'))}</span>`:''}</h1><p>${esc(config.subtitle)}</p>${searchForm()}<div class="pub-hot"><span>热门主题</span>${['耕地','矿业权','生态修复'].map(q=>link('search',q,'','',{q})).join('')}${btn('智能帮办','portalAssistant','','text small','sparkles')}</div></div>
        <div class="home-map-story"><div class="home-map-heading"><span>SPATIAL ATLAS</span><small>山水林田湖草沙 · 一图共览</small></div>${ctx.landscape()}<a class="home-map-entry" href="/gis/index.html"><span><strong>在地图上，发现更多</strong><small>查看图层 · 查询对象 · 空间分析</small></span>${icon('arrow')}</a></div>
      </div></section>
      <div class="portal-container home-content">
        <div class="pub-quick">${[['map','地图浏览','查看空间分布','layers'],['data','数据申请','发现与获取数据','database'],['capabilities','常用工具','在线分析与量算','tool'],['services','办事服务','查询、指南与咨询','file']].map(([r,n,d,i],index)=>link(r,`<small class="home-quick-number">0${index+1}</small><span class="tile">${icon(i)}</span><span><strong>${n}</strong><small>${d}</small></span>${icon('arrow')}`,'','pub-quick-item')).join('')}</div>
        <section class="pub-notifications"><strong>${icon('bell')}政策与通知</strong><div>${news.filter(r=>['政策法规','行业动态'].includes(newsCategory(r))).slice(0,2).map(r=>`<button data-action="portalKnowledge" data-id="${esc(r.id)}"><span>${esc(r.name)}</span><time>${esc(r.updated?.slice(0,10))}</time></button>`).join('')||'<span>暂无已发布通知</span>'}</div>${link('knowledge','更多 '+icon('arrow'),'','pub-more')}</section>
        <section class="pub-section home-data"><span class="home-section-kicker">01 / DATA & RESOURCES</span>${section(selected.length?'推荐数据':'最新数据','从资源目录出发，找到业务需要的数据。','data')}<div class="home-resource-grid">${recommended.map(resourceCard).join('')}</div></section>
        <section class="pub-section pub-service-columns"><div class="home-tools"><span class="home-section-kicker">02 / SPATIAL TOOLS</span>${section('常用能力','让专业的空间工具，成为顺手的日常能力。','capabilities')}<div class="pub-tool-shortcuts">${tools().filter(t=>t.available).map(t=>link('capabilities',`<span class="tile">${icon(t.icon)}</span><span><strong>${esc(t.name)}</strong><small>${esc(t.description)}</small></span>${icon('arrow')}`,t.id,'pub-tool-shortcut')).join('')}</div></div><div class="home-services"><span class="home-section-kicker">03 / PUBLIC SERVICES</span>${section('便民办事','办事有指引，进度可查询。','services')}<div class="pub-service-links">${ordered(P().services).slice(0,6).map((s,i)=>link('services',`<small>${String(i+1).padStart(2,'0')}</small><span>${esc(s.name)}</span>${icon('arrow')}`,s.id,'')).join('')}</div><div class="home-service-help"><span>${icon('sparkles')}不知道从哪里开始？</span>${btn('智能帮办','portalAssistant','','text small','arrow')}</div></div></section>
        <section class="pub-section home-scenic"><span class="home-section-kicker">04 / DISCOVER INNER MONGOLIA</span>${section('大美内蒙古','从辽阔草原到北疆湿地，看见每一寸土地的美。','landscape')}<div class="pub-topics">${ordered(P().topics).map(topicCard).join('')}</div></section>
        <section class="pub-section home-news"><span class="home-section-kicker">05 / NEWS & GUIDES</span>${section('资讯与资料','了解政策动态，获取业务指南。','knowledge')}<div class="pub-news-list">${news.slice(0,4).map(r=>`<button data-action="portalKnowledge" data-id="${esc(r.id)}"><time>${esc(r.updated?.slice(0,10)||'')}</time><small>${esc(newsCategory(r))}</small><strong>${esc(r.name)}</strong>${icon('arrow')}</button>`).join('')}</div></section>
      </div></div>`;
  }
  function search(){
