...
export function mountAnalysis(host,ctx,mode='compliance'){
  const {api,esc}=ctx;let disposed=false,map=null,timer=null,current=null,catalog=null,input=null,busy=false,drawing=null;
  ...
  function paint(data,conflicts={features:[]}){if(!map)return;map.setAnalysisFeatures([...catalog.layers.map(l=>({type:'Feature',geometry:l.geometry,properties:{kind:'constraint',name:l.name}})),...(data?.features||[]).map(f=>({...f,properties:{...f.properties,kind:'input'}})),...conflicts.features]);}
  ...
  function show(job){
    ...
    paint(r.input,r.conflicts);fit(r.input);
  }
  ...
  host.addEventListener('input',e=>{input=null;if(e.target.id==='ana-geometry'){invalidateDraft();map?.setAnalysisFeatures([]);listParcels();$('#ana-editor').hidden=true;}});
  ...
  host.addEventListener('click',async e=>{
    ...
    try{
      ...
      if(action==='sample'){$('#ana-geometry').value=JSON.stringify(catalog.sample,null,2);$('#ana-crs').value='EPSG:4326';input=null;setDraft(catalog.sample);fit(catalog.sample);message('已载入一块冲突地块和一块无冲突地块。');}
        ...
        if(disposed)return;host.spatialResult=r;map?.setAnalysisFeatures(r.features.map(f=>({...f,properties:{...f.properties,kind:'input'}})));if(r.features.length)fit(r);message(r.empty?'计算完成，结果为空。':`计算完成：融合去重面积 ${(r.areaSquareMeters/10000).toFixed(4)} 公顷。`);$('#ana-spatial-export').hidden=false;
      }
  ...
  (async()=>{
    ...
    if(disposed){map.destroy();return;}listParcels();paint(null);map.fitGeometry({type:'FeatureCollection',features:catalog.layers.filter(l=>l.geometry).map(l=>({type:'Feature',geometry:l.geometry,properties:{}}))});if(ctx.initialGeometry){$('#ana-crs').value='EPSG:4326';setDraft(ctx.initialGeometry);fit(ctx.initialGeometry);message('已载入地图选中地块，请校验或运行核查。');}await history();
